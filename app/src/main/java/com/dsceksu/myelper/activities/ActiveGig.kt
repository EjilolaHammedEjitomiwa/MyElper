...
import kotlinx.android.synthetic.main.activity_active_products.*
import me.solidev.loadmore.AutoLoadMoreAdapter
import me.solidev.loadmore.AutoLoadMoreConfig

class ActiveGig : AppCompatActivity() {
    ...
    private var isViewSellerProduct: Boolean? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        isViewSellerProduct = intent.getBooleanExtra(Constants.isViewSellerProduct, false)

        productAdapter = if (sellerID == Utils.currentUserID()) {
            ProductAdapter(this, productList, true, false)
        } else {
            ProductAdapter(this, productList, false, false)
        }
        ...
        active_product_recyclerView.setHasFixedSize(true)
        val productLayoutManager = GridLayoutManager(this, 2)
        active_product_recyclerView.layoutManager = productLayoutManager
        active_product_recyclerView.adapter = autoLoader

        Utils.showLoader(this,"Loading")
        getProductList()

        if (isViewSellerProduct!!) {
            loadSellerUsername()
        }
        ...
        Utils.database().collection(Constants.users)
                ...
                .addOnSuccessListener {
                    if (it.exists()) {
                        ...
                        active_product_toolbarTitle.text = "${seller!!.username} products"
                    }
                }
    }
        ...
        query.get().addOnSuccessListener { it1 ->
            ...
            if (it1.size() > 0) {
                ...
                for (snapshot in it1) {
                    ...
                    if (products.category != Constants.service) {
                        productList.add(products)
                    }
                }

                if (productList.isEmpty()) {
                    Toasty.info(this, "No product found", Toasty.LENGTH_LONG).show()
                }
    ...
    override fun onResume() {
        super.onResume()
        Utils.setUserOnline()
        lastVissiblePosition = (active_product_recyclerView.layoutManager as LinearLayoutManager).findFirstCompletelyVisibleItemPosition()
        (active_product_recyclerView.layoutManager as LinearLayoutManager).scrollToPosition(lastVissiblePosition)
    }
